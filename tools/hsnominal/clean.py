...
def clean(imagepath):
    ...
    with Image(filename=imagepath) as img:
        ...
        with Color("black") as black:
            ...
            # Add border
            command.append("-bordercolor black -border 10x10")
            ...
            # make grayscale
            command.append("-colorspace gray")

            command.append("'%s.clean8.jpg'" % imagepath)

            subprocess.run(" ".join(command), shell=True, stdout=subprocess.PIPE)

            #cleanup with Textcleaner (see http://www.fmwconcepts.com/imagemagick/textcleaner/)
            command = "/path/to/textcleaner -e normalize -f 20 -o 5 '%s.clean8.jpg' '%s.clean9.jpg'" % (imagepath, imagepath)
